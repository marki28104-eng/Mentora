...
def get_notes(courseId: int,
            chapterId: int, db: Session = Depends(get_db),
            current_user: User = Depends(get_current_active_user)):

    notes = db.query(Note).filter(Note.course_id == courseId,
                                  Note.chapter_id == chapterId,
                                  Note.user_id == current_user.id).all()
    if not notes:
        raise HTTPException(status_code=404, detail="No notes found for this course and chapter")
...
def add_note(note: NoteCreate, db: Session = Depends(get_db),
    ...
    db_note = Note(course_id=note.courseId, chapter_id=note.chapterId, user_id=current_user.id, text=note.text)
    db.add(db_note)
    db.commit()
    db.refresh(db_note)
